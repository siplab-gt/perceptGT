...
0. Main file is the "perceptGT.m" file, user will only need to interact with this file. The user can select **a** single folder or multiple JSON files, when running the main file in 1. **as long as they are inside the parent folder(Percept_files) or the parent folder itself.**
